...
const modes = [
  {
    ...
    subtitle: "Self-care · Journaling · Healing",
  },
  {
    id: "friend",
    Icon: Users,
    label: "Friend Mode",
    subtitle: "2–8 players · Room code",
  },
  {
    id: "couple",
    Icon: Heart,
    label: "Couple Mode",
    subtitle: "2 players · Deep talks",
  },
  {
    id: "family",
    Icon: HomeIcon,
    label: "Family Mode",
    subtitle: "2–10 players · Bonding",
  },
];
          ...
          {modes.map((mode) => {
            ...
            return (
              <motion.button
                ...
                className={`hover-card border rounded-xl p-6 sm:p-8 text-left w-full select-none
                            flex flex-row items-center justify-between group
                            ${isSelected ? "mode-selected" : ""}`}
              >
                <div className="flex flex-col">
                  <h3
                    className="card-title font-crimson font-medium text-2xl tracking-wide mb-1 transition-colors"
                    style={{ color: "var(--text-main)" }}
                  >
                    {mode.label}
                  </h3>
                  <p
                    className="card-sub font-mono text-[9px] uppercase tracking-[0.18em] transition-colors"
                    style={{ color: "var(--text-accent)" }}
                  >
                    {mode.subtitle}
                  </p>
                ...
                >
                  <Icon
                    size={18}
                    strokeWidth={1.5}
                    className="card-icon opacity-70 transition-all duration-500 group-hover:scale-110"
                    style={{ color: "var(--text-main)" }}
                  />
                </div>
              </motion.button>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4">
          <div>
            {selected ? (
              ...
              >
                <span
                  className="font-mono text-[9px] uppercase tracking-[0.2em] block"
                  style={{ color: "var(--text-accent)" }}
                >
                  Selected
                </span>
                <span
                  className="font-crimson font-medium text-lg"
                  style={{ color: "var(--text-main)" }}
                >
          ...
          <motion.button
            ...
            disabled={!selected}
            className="primary-action flex items-center gap-2 h-10 px-7 rounded-full font-mono
                       text-[10px] uppercase tracking-[0.2em] select-none transition-opacity"
            style={{
