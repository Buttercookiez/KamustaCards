...
import { roomSteps } from "@/lib/tourSteps";
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 h-20 backdrop-blur-md" style={{ background: "var(--bg-nav)" }}>
        <div className="max-w-4xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            <span data-tour="room-code-display" className="font-mono text-[9px] uppercase tracking-[0.3em] opacity-50" style={{ color: "var(--text-main)" }}>
              {room.roomCode}
            </span>
            {streak > 0 && (
          ...
          {showDraw && (
            <motion.div key="solo-draw" variants={phaseVariants} initial="initial" animate="animate" exit="exit"
              ...
              <div className="text-center w-full flex flex-col items-center">
                ...
                <motion.div data-tour="current-card" className="my-10">
                  <KamustaCard
              ...
              {!submitted ? (
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="pt-6 border-t flex items-center justify-between flex-wrap gap-4" style={{ borderColor: "var(--border-line)" }}>
                    ...
                    <button data-tour="next-card-btn" onClick={nextCard}
...
function MultiplayerRoom({ room, roomId, currentUser, theme, toggleTheme }: {
  ...
  // 🛡️ DEFENSIVE FIX: Convert any accidental database objects back to strings
  const safePlayers = (room.players || []).map((p: any, i: number) => 
    typeof p === "string" ? p : (p?.name || p?.username || p?.id || `Player${i}`)
  );
  
  const safeActivePlayers = (room.activePlayers || []).map((p: any, i: number) => 
    typeof p === "string" ? p : (p?.name || p?.username || p?.id || `Player${i}`)
  );
  ...
  useEffect(() => {
    ...
    // 🛡️ USING SAFE DATA
    const active = safeActivePlayers.length ? safeActivePlayers : safePlayers;
  ...
  }, [room.status, room.answers, safeActivePlayers, safePlayers, room.currentDrawer, currentUser, roomId]);
  ...
  const handleContinue = async () => {
    ...
    // 🛡️ USING SAFE DATA
    const activePlayers = safeActivePlayers.length ? safeActivePlayers : safePlayers;
    ...
    if (activePlayers?.every((p: string) => updated.includes(p))) {
      ...
      await updateDoc(doc(db, "rooms", roomId), {
        ...
        currentDrawer: nextDrawer || players[0], // Fallback safely
      });
    }
  };
    ...
    <div className="min-h-screen grain-overlay font-crimson flex flex-col px-6">
      ...
      <main className="flex-1 flex flex-col pt-28 pb-16 max-w-2xl mx-auto w-full relative z-10">
        <div className="mb-12 border-b pb-4 flex flex-wrap gap-4" style={{ borderColor: "var(--border-line)" }}>
          {/* 🛡️ REPLACED room.players WITH safePlayers */}
          {safePlayers.map((p: string) => (
          ...
          {room.status === "revealed" && room.currentCard && (
            <motion.div key="revealed" variants={phaseVariants} initial="initial" animate="animate" exit="exit"
              ...
              <div className="flex flex-col gap-10 flex-1">
                ...
                {/* 🛡️ REPLACED room.players WITH safePlayers */}
                {safePlayers.map((p: string, i: number) => {
                ...
                {isCouple && (
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    ...
                    {!iHaveClickedContinue ? (
                      <button data-tour="next-card-btn" onClick={handleContinue}
...
export default function RoomClient({ roomId }: { roomId: string }) {
  const router = useRouter();
  const tour = useOnboardingTour("room");
  ...
  return (
    <>
      {room.mode === "solo" ? (
        <SoloRoom {...sharedProps} />
      ) : (
        <MultiplayerRoom {...sharedProps} />
      )}
      ...
      <OnboardingTour 
        ...
        onNext={() => tour.next(roomSteps.length)} 
        onPrev={tour.prev} 
        onSkip={tour.skip} 
