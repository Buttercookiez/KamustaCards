...
import {
  ...
  Users, Heart, UserCircle, Home, Loader2
} from "lucide-react";
...
function LobbyClient() {
  ...
  // Safe check for URL params (handles both ?roomId= or ?id=)
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "rooms", roomId), (snap) => {
      ...
      // Redirect to the Game Room if the game is already in progress
      if (IN_GAME.includes(data.status)) {
        setRejoining(true);
        // FIX: Redirect to /room, NOT /lobby!
  ...
  // FIXED: Show loading instead of white screen if data is fetching
  if (!mounted || !room || !roomId) {
    return (
      <div className="min-h-screen grain-overlay flex flex-col items-center justify-center gap-4">
        <GlobalStyles />
        <Loader2 className="w-5 h-5 animate-spin opacity-50" style={{ color: "var(--text-main)" }} />
        <span className="font-mono text-[10px] uppercase tracking-widest opacity-50" style={{ color: "var(--text-main)" }}>
          Connecting to Lobby...
        </span>
      </div>
    );
