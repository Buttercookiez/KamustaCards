...
const GlobalStyles = () => (
  <style jsx global>{`
    ...
    /* Remove default blue tap highlight on mobile/touch */
    * {
      -webkit-tap-highlight-color: transparent !important;
    }

    /* Remove default blue focus outline, replace with theme color for accessibility */
...
useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // Safety catch: If they bypass onboarding and have NO name
      if (!u.displayName && !hasCompletedOnboarding) {
  ...
  return (
    <div className="min-h-screen grain-overlay font-crimson transition-colors duration-500">
      <GlobalStyles />
      ...
      <main className="pt-32 pb-20 px-4 sm:px-6 max-w-5xl mx-auto relative z-10">
        ...
        >
          ...
          <h1 
            className="text-4xl sm:text-5xl font-crimson font-medium tracking-tight"
            style={{ color: 'var(--text-main)' }}
          >
        ...
        {/* Quick Actions Grid - Tactile & Interactive */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-16">
          ...
          >
            <div className="flex flex-col">
              <h3 
                className="font-crimson font-medium text-2xl tracking-wide mb-1"
            ...
            <div className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--bg-base)]/10 group-hover:bg-[var(--bg-base)]/20 transition-colors duration-400">
              <Plus 
                className="w-5 h-5 transition-transform duration-500 group-hover:rotate-90 group-hover:scale-110" 
                style={{ color: 'var(--action-text)' }}
                strokeWidth={1.5}
              />
            </div>
          ...
          >
            ...
            <div className="w-10 h-10 rounded-full border border-[var(--border-subtle)] group-hover:border-[var(--text-main)] flex items-center justify-center transition-colors duration-400">
              <Globe 
                className="w-5 h-5 transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110 opacity-70 group-hover:opacity-100" 
                style={{ color: 'var(--text-main)' }}
                strokeWidth={1.5}
              />
            </div>
        ...
        <div>
          ...
          >
            <h2 
              className="font-crimson text-xl sm:text-2xl font-medium"
              style={{ color: 'var(--text-main)' }}
            >
              Your Library
            </h2>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              <div 
                className="w-6 h-6 rounded-full border-t-2 animate-spin" 
                style={{ borderColor: 'var(--text-accent)' }}
              />
            </div>
          )}

          {/* Empty State / Seed Decks */}
          {!loading && decks.length === 0 && !seeded && (
            ...
            >
              <div className="max-w-md mx-auto text-center">
                <div 
                  className="w-12 h-12 mx-auto rounded-full flex items-center justify-center mb-6"
                  style={{ backgroundColor: 'var(--icon-bg)' }}
                >
                  <Sparkles className="w-5 h-5" style={{ color: 'var(--text-accent)' }} strokeWidth={1.5} />
                </div>
                <h3 
                  className="font-crimson text-2xl mb-3 font-medium transition-colors"
                  style={{ color: 'var(--text-main)' }}
                >
                  Start your collection
                </h3>
                <p 
                  className="font-crimson text-sm leading-relaxed mb-8 transition-colors"
                  style={{ color: 'var(--text-sub)' }}
                >
            ...
            >
              {decks.map((deck) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <h4 
                      className="font-crimson font-medium text-xl leading-tight line-clamp-1 transition-colors group-hover:text-[var(--text-accent)]"
                      style={{ color: 'var(--text-main)' }}
                    >
                      {deck.title}
                    </h4>
                  ...
                  <div className="flex items-end justify-between">
                    <p 
                      className="font-crimson text-sm line-clamp-2 leading-relaxed opacity-70 flex-1 pr-4"
                      style={{ color: 'var(--text-sub)' }}
                    >
                      {deck.description}
                    </p>
              ...
              >
                <div className="w-10 h-10 rounded-full flex items-center justify-center mb-3 transition-colors duration-300 group-hover:bg-[var(--icon-bg)]">
                  <Plus 
                    className="w-5 h-5 opacity-50 group-hover:opacity-100 transition-all duration-500 group-hover:rotate-90 group-hover:scale-110" 
                    style={{ color: 'var(--text-main)' }}
                    strokeWidth={1.5}
                  />
                </div>
                <span 
                  className="font-mono text-[10px] uppercase tracking-[0.2em] font-medium opacity-60 group-hover:opacity-100 transition-opacity"
                  style={{ color: 'var(--text-main)' }}
                >
                  Create New Deck
                </span>
